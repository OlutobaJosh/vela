    ...
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: 'var(--bg)' }}>
      ...
      <div className="relative w-full max-w-sm anim-up">
        ...
        <div className="surface p-7">
          <h1 className="font-semibold text-lg mb-1 tracking-tight" style={{ color: 'var(--text)' }}>Create your account</h1>
          <p className="text-xs mb-6" style={{ color: 'var(--text-3)' }}>Start your 14-day free trial. No credit card needed.</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div>
              ...
              <input type="password" value={password} onChange={e => setPassword(e.target.value)}
                placeholder="Min. 8 characters" minLength={8} required className="field" />
            </div>
        ...
        <p className="text-center text-xs mt-5" style={{ color: 'var(--text-3)' }}>
          ...
          <Link href="/login" className="transition-colors hover:text-white" style={{ color: 'var(--accent)' }}>Sign in</Link>
        </p>
      </div>
    </div>
  );
